...
void ImageSegDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  vector<int> top_shape = this->data_transformer_->InferBlobShape(cv_img);
  this->transformed_data_.Reshape(top_shape);
  this->transformed_label_.Reshape(top_shape);
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    //LOG(INFO) << "line: " << root_folder + lines_[lines_id_].first;
    //LOG(INFO) << "line: " << root_folder + lines_[lines_id_].second;
    ...
    CHECK(cv_img_seg[0].data) << "Could not load " << lines_[lines_id_].first;
    CHECK(cv_img_seg[1].data) << "Could not load " << lines_[lines_id_].second;
    //LOG(INFO) << "label row: " << cv_img_seg[1].rows << ", cols: " << cv_img_seg[1].cols << ".";
