...
if create
    ...
    h5create(filename, '/data', [dat_dims(1:end-1) Inf], 'Datatype', Datatype, 'ChunkSize', [dat_dims(1:end-1) chunksz]); % width, height, channels, number
    h5create(filename, '/label', [lab_dims(1:end-1) Inf], 'Datatype', Datatype, 'ChunkSize', [lab_dims(1:end-1) chunksz]); % width, height, channels, number
...
if ~isempty(data)
    h5write(filename, '/data', data, startloc.dat, size(data));
